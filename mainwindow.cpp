...

{
  ...
  ui->label->setText(
      "You feel a dark desire to BUUUUURRRRRNNNN!!!\nFind a way to burn down "
      "every house by\ncrafting different items and placing them in a "
      "house!\nBe careful though as you might just burn yourself\nso be sure "
      "to go straight to the exit once you start a fire\nDon't the way out");
  QMediaPlaylist *playlist = new QMediaPlaylist();
  playlist->addMedia(QUrl("qrc:/resource/Just Cause 3 Soundtrack - Torre "
                          "Florim - Firestarter (Intro Song).mp3"));
...
void MainWindow::on_NorthBtn_clicked() { move("north"); }

void MainWindow::on_EastBtn_clicked() { move("east"); }

void MainWindow::on_WestBtn_clicked() { move("west"); }

void MainWindow::on_SouthBtn_clicked() { move("south"); }
...
void MainWindow::fillList() {
  ...
  for (size_t i = 0; i < inventory.size(); i++)
    ui->inventoryList->addItem(
        QString::fromStdString(inventory[i].getShortDescription()));
  for (size_t i = 0; i < roomItems.size(); i++)
    ui->roomItemList->addItem(
        QString::fromStdString(roomItems[i].getShortDescription()));
}

void MainWindow::craftChange() { craftBool = false; }

bool MainWindow::getCraft() { return craftBool; }
...
void MainWindow::on_take1Button_clicked() {
  if (ui->roomItemList->currentIndex().row() != -1) {
    ui->inventoryList->addItem(
        QString::fromStdString(roomItems[ui->roomItemList->currentIndex().row()]
                                   .getShortDescription()));
...
void MainWindow::on_placeButton_clicked() {
  if (ui->inventoryList->currentIndex().row() != -1) {
    if (current->getType() == 8 && !h->getFire() &&
        inventory[ui->inventoryList->currentIndex().row()].getMod() == 5) {
      h->setFire();
      outside->HouseFire(h);
      current->setNextFire();
      ui->label->setText("The house is on fire and your power grows\nQuit "
                         "stalling and get out");
      win++;
      if (win > 5) {
        gameWin();
      }
      ...
      ui->roomItemList->addItem(QString::fromStdString(
          inventory[ui->inventoryList->currentIndex().row()]
              .getShortDescription()));
    ...
    inventory.erase(inventory.begin() +
                    ui->inventoryList->currentIndex().row());
...
bool MainWindow::getOpen() { return open; }

void MainWindow::finish() { QTimer::singleShot(1000, this, SLOT(close())); }

void MainWindow::gameFail() {
  QMessageBox::StandardButton resBtn = QMessageBox::question(
      this, "Zork", tr("You Lose\nYou burned yourself in a fiery glory!\n"),
