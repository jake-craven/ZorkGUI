...
House::House(string description, int rId, int type, Space *s)
...
void House::RandomRoomGenerator(int rId) {
  ...
  for (int i = 0; i < nRooms; nRooms--) {
    ...
    if (RoomMap[x][y] == NULL) {
      if (x == 4 || y == 4 || x == 0 || y == 0) {
        if (x != y) {
          ...
          case 0:
            ...
            break;
          case 4:
            if (RoomMap[x - 1][y] != NULL || RoomMap[x + 1][y] != NULL ||
                RoomMap[x][y - 1] != NULL) {
              RoomMap[x][y] = roomList[nRooms];
              RoomMap[x][y]->setX(x);
              RoomMap[x][y]->setY(y);
      ...
      else if (RoomMap[x + 1][y] != NULL || RoomMap[x - 1][y] != NULL ||
               RoomMap[x][y + 1] != NULL || RoomMap[x][y - 1] != NULL) {
        RoomMap[x][y] = roomList[nRooms];
        RoomMap[x][y]->setX(x);
        RoomMap[x][y]->setY(y);
      } else if (count == 0) {
      ...
      if (RoomMap[x][y] != NULL) {
        if (x == 4 || y == 4 || x == 0 || y == 0) {
          if (x != y) {
            switch (x) {
            case 4:
              RoomMap[x][y]->setExits(RoomMap[x - 1][y], RoomMap[x][y + 1],
                                      NULL, RoomMap[x][y - 1]);
              break;
            case 0:
              RoomMap[x][y]->setExits(NULL, RoomMap[x][y + 1],
                                      RoomMap[x + 1][y], RoomMap[x][y - 1]);
              break;
            }
            switch (y) {
            case 0:
            ...
            case 4:
              ...
              RoomMap[x][y]->setExits(RoomMap[x - 1][y], NULL,
                                      RoomMap[x + 1][y], RoomMap[x][y - 1]);
              break;
            }
          } else {
            switch (y) {
            case 4:
              RoomMap[x][y]->setExits(RoomMap[x - 1][y], NULL, NULL,
                                      RoomMap[x][y - 1]);
...
int House::getX() { return x1; }
int House::getY() { return y1; }
