...
CraftDialog::CraftDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::CraftDialog) {
  ui->setupUi(this);
  ui->craftedItem->setText("");
  ui->craftButton->setHidden(true);
}

CraftDialog::~CraftDialog() { delete ui; }
  ...
  if (!craftChange) {
    ...
    if ((*inventory).size() >= index1) {
      if ((*inventory)[index1].getMod() < 4)
        (*inventory)
            .erase((*inventory).begin() + index1,
                   (*inventory).begin() + index1 + 1);
    }
    if ((*inventory).size() >= index2) {
      if ((*inventory)[index2].getMod() < 4)
        (*inventory)
            .erase((*inventory).begin() + index2,
                   (*inventory).begin() + index2 + 1);
    }
    (*inventory).push_back(i);
    ui->dropMenu1->addItem(QString::fromStdString(
        (*inventory)[(*inventory).size() - 1].getShortDescription()));
    ui->dropMenu2->addItem(QString::fromStdString(
        (*inventory)[(*inventory).size() - 1].getShortDescription()));
    if (index1 > index2) {
...
void CraftDialog::on_cancelButton_clicked() { this->close(); }
...
void CraftDialog::on_dropMenu1_currentIndexChanged(int index) {
  ...
  if (indexO > -1) {
    ...
    this->ui->craftedItem->setText(
        QString::fromStdString(craft(index, indexO)));
  } else
    ui->craftButton->setHidden(true);
}
...
void CraftDialog::addInventory(vector<Item> *inv) { inventory = inv; }
  ...
  if ((*inventory)[index].getHold() || (*inventory)[index0].getHold()) {
    ...
    if (first == 1 && that == 2) {
      if ((*inventory)[index0].getShortDescription() == "Vodka") {
        ...
        return (*inventory)[index0].getShortDescription() + " soaked " +
               (*inventory)[index].getShortDescription();
    } else if (first == 2 && that == 1) {
      if ((*inventory)[index].getShortDescription() == "Vodka") {
        return "Vodka Molotov";
      } else
        return (*inventory)[index].getShortDescription() + " soaked " +
               (*inventory)[index0].getShortDescription();
    } else if (first == 3 && that >= 4) {
...
void CraftDialog::closeEvent(QCloseEvent *event) { open = false; }

void CraftDialog::setFalse() { craftChange = false; }

bool CraftDialog::getCraftChange() { return craftChange; }
